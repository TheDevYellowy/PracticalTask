...
public class HideAndSeek extends Game {
    ...
    if (this.isInGracePeriod()) {
      for (Player seeker : this.seekers) {
        if (!seeker.hasPotionEffect(PotionEffectType.BLINDNESS))
          seeker.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 999, 4, false, false));
      }
    }
  }
  ...
  void onDamageEvent(EntityDamageByEntityEvent event) {
    ...
    if(!this.isInsideArea(damaged) || !this.isInsideArea(damager)) return;
    ...
    if (this.hiders.contains(hunter) && !event.isCancelled()) event.setCancelled(true);
